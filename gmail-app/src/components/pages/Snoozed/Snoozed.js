...
const Snoozed = () => {
  ...
  const toggleSelectAll = () => {
    if (selectedEmails.length === snoozedEmails.length) {
      setSelectedEmails([]);
    } else {
      setSelectedEmails(snoozedEmails.map((email) => email.id));
    }
  };

  return (
    <div className="mailbox">
      <h2>snoozed Emails</h2>

      {/* Header Controls */}
      <div className="email-header">
        <button className="icon-btn" onClick={toggleSelectAll}>
          {selectedEmails.length === snoozedEmails.length ? (
            <CheckSquare size={18} />
          ) : (
            <Square size={18} />
          )}
      ...
      <div className="email-list">
        {snoozedEmails.map((email) => (
          ...
          >
            ...
            <button className="icon-btn">
              <Star size={16} color={email.snoozed ? "gold" : "gray"} />
            </button>
